...
// Cache object to prevent duplicate lookups for the same coordinates
...
function LocationCell({ lat, lon }: { lat: number; lon: number }) {
  ...
  // Initialize state directly to avoid calling setState synchronously in useEffect
  ...
  // Calculate distance from Firefly Crafts PH shop pin
  const distanceMeters = calculateShopDistance(lat, lon);
  const isAtShop = distanceMeters >= 0 && distanceMeters <= 150; // Within 150m allowance
